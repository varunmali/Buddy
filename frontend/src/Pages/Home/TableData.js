...
const TableData = (props) => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    formData.append("title", values.title || editingTransaction[0].title);
    formData.append("amount", values.amount || editingTransaction[0].amount);
    formData.append(
      "description",
      values.description || editingTransaction[0].description
    );
    formData.append(
      "category",
      values.category || editingTransaction[0].category
    );
    formData.append("date", values.date || editingTransaction[0].date);
    formData.append(
      "transactionType",
      values.transactionType || editingTransaction[0].transactionType
    );

    // Handle image file if a new file is selected
    if (values.image) {
  ...
  const handleChange = (e) => {
    if (e.target.name === "image") {
      setValues({ ...values, image: e.target.files[0] }); // Handle file input
    } else {
      setValues({ ...values, [e.target.name]: e.target.value });
    }
  };
